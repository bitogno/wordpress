...
class Opalestate_Field_User_upload {
	...
	public static function init() {
		...
		add_action( 'wp_ajax_opalestate_user_upload',  array(__CLASS__,'upload_image') );
		add_action( 'wp_ajax_nopriv_wp_ajax_opalestate_user_upload',  array(__CLASS__,'upload_image') );
  ...
  $ajax_response = array( 'success' => false, 'reason' => 'Image upload failed!' );
        ...
        do_action( 'opalestate_before_process_ajax_user_upload_file' );
	...
	public static function render_map( $field, $field_escaped_value, $field_object_id, $field_object_type, $field_type_object ) {
		...
		echo '<div class="opalestate-user-upload '.apply_filters('opalestate_row_container_class', 'opal-row').'"> '; ?>
				<div class="upload-container" data-button="select-images<?php echo $id; ?>" data-field="<?php echo $field->args( '_name' );?>">
					<?php if( $files ): ?>
						...
						<?php 
	                        echo '<div class="upload-thumbnail">';
	                            ...
	                            echo '<a class="icon icon-delete" data-toggle="tooltip" title="'.__('Delete','opalestate').'" data-post-id="'.intval($post_id).'" data-attachment-id="'.intval($image_id).'" href="javascript:;">';
						...
						<?php endforeach; ?>
					<?php endif ; ?>
				    <a id="select-images<?php echo $id; ?>"   href="javascript:;" class="button-upload"> <span>+</span>	 <?php // esc_html_e( 'Select Images', 'opalestate' ); ?></a>
			</div> 	
		<?php echo '</div>';	
	}
	...
	public static function render_map_single( $field, $field_escaped_value, $field_object_id, $field_object_type, $field_type_object ) {
		...
		echo '<div class="opalestate-user-upload '.apply_filters('opalestate_row_container_class', 'opal-row').'"> '; ?>
				<div class="upload-container single-upload" data-button="select-images<?php echo $id; ?>" data-field="<?php echo $field->args( '_name' );?>" data-issingle="1">
					<?php if( $files ): ?>
						<?php foreach( $files as $image_id => $url ): ?>
						<div class="upload-item">
						<?php 
	...
	public static function setup_admin_scripts() {
		wp_enqueue_script( 'opalestate-userupload', plugins_url( 'assets/script.js', __FILE__ ), array(  ), self::VERSION );
		wp_enqueue_style( 'opalestate-userupload', plugins_url( 'assets/style.css', __FILE__ ), array(), self::VERSION );
        ...
        wp_localize_script( 'opalestate-userupload', 'opalestate', $prop_data );

	}
}

Opalestate_Field_User_upload::init();
