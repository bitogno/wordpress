...
class OpalEstate_Membership{
	...
	public static function render_membership_free_package(){
		echo Opalestate_Template_Loader::get_template_part( 'parts/membership-free-package', array() );
	}
	...
	public static function check_ajax_upload(){
		...
		if( ! $check || ! $has ){
			$std = new stdClass();
			$std->status = false ;
			$std->message = __( 'Could not allow uploading image','opalestate' );
	...
	public static function metabox( $fields ) {
		...
		$fields[] = array(
			...
			'type' => 'checkbox',
			'std'  => '1',
			'description' => __( 'No, it is not unlimited, If not set it will be unlimited. Number of properties can make featured with this package.', 'opalestate' )
		);
	...
	public static function render_membership_summary($package_id=0, $payment_id=0){
	    ...
	    if( $unlimited_listings == 1 && $package_id > 0 ){
	    	$output .= '<li><span>'.__('(Package) Listings Included:','opalestate').'</span> '.__( 'Unlimited', 'opalestate' ).'</span></li>';
	    	$output .= '<li><span>'.__('(Package) Featured Included:','opalestate').'</span> '.__( 'Unlimited', 'opalestate' ).'</li>';
	    }else {
	    	if( $package_id > 0 ){
		...
		//if( (isset($_GET['user_id']) && $_GET['user_id']) ){
			...
			$cmb_user = new_cmb2_box( array(
	 		...
	 		$fields[] = array(
					...
					'std' => '1',
					'description' => __( 'Set package ID with -1 as free package.', 'opalestate' ),
					'before_row' => '<hr><h3> '.__( 'Membership Information', 'opalestate' ).' </h3>'
				);
			...
			$fields[] = array(
				'name' => __( 'Expired', 'opalestate' ),
				'id'   => $prefix . 'package_expired',
				'type' => 'text',
				'std' => '1',
				'description' => __( 'Show expired time in double format.', 'opalestate' )
			);
