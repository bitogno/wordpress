...
*/
     ...
     */
     ...
     // Recherche d'agences
    if ($('#opalestate-map-agents').length > 0) {
        $.ajax({
            type: 'GET',
            dataType: 'json',
            url: opalesateJS.ajaxurl,
            data: location.search.substr(1) + "&action=opalestate_ajx_get_agents",
            success: function(data) {
                initializeAgentsMap(data);
    ...
    // Fonction Javascript qui initialise la carte et ajoute les markers obtenus via AJAX
    // A COMPLETER (le fonctionnement est là, mais pas l'apparence des markers)
    // Ne fonctionne pas avec la BDD actuelle (agents à la place des agences et agents en double = array de valeurs pour chaque attribut)
    function initializeAgentsMap(properties) {
        ...
        var map = new google.maps.Map(document.getElementById("opalestate-map-agents"), mapOptions);
        ...
        for (var i = 0; i < properties.length; i++) {
            ...
            // Ci-dessous : ne fonctionne pas avec la BDD actuelle (agents en double = tableau de valeurs pour chaque objet)
            // ex : pour que ça fonctionne, faire properties[i].opalestate_agt_avatar[0] (ceci est un tableau car attribut en double, mais ne le sera pas plus tard)
            var url = properties[i].opalestate_agt_avatar;
            var size = new google.maps.Size(42, 57);
            if (window.devicePixelRatio > 1.5) {
            ...
            markers[i] = new google.maps.Marker({
                position: new google.maps.LatLng(properties[i].opalestate_agt_map_latitude, properties[i].opalestate_agt_map_longitude),
                map: map,
                icon: image,
            ...
            // Ci-dessous HTML à adapter pour les agences
