...
function fullhouse_fnc_breadcrumbs() {
	...
	if (!is_home() && !is_front_page() || is_paged()) {
		...
		if (is_category()) {
			...
			$title = esc_html__( 'Blogs', 'fullhouse' );
		} elseif (is_day()) {
			...
			if ( get_post_type() != 'post' ) {
				...
				if ( get_post_type() == 'opalestate_property' ) {
					$title  = esc_html__( 'Single Property', 'fullhouse' );
				} elseif ( get_post_type() == 'opalestate_agent' ) {
					$title  = esc_html__( 'Single Agent', 'fullhouse' );
				} elseif ( get_post_type() == 'opalestate_office' ) {
					$title  = esc_html__( 'Single Office', 'fullhouse' );
				} else {
					$title = get_the_title();
				}
			} else {
				...
				$breadcrumbs_html .= trim($before) . get_the_title() . $after;
				$title  = esc_html__( 'Blog', 'fullhouse' );
			}
